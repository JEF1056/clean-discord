...
def run_detox(to_clean):
    #precompute tasks and create required dir
    if args.overwrite: tasks=os.listdir(to_clean); print(f"Overwriting data in {to_clean}")
    else: tasks=[m for m in os.listdir(to_clean) if m not in os.listdir(to_clean+"-detox")]; print(f"Found {len(os.listdir(args.dir))-len(tasks)} files in {to_clean}, skipping.")
